...
const serviceImplementation = {
    ...
    sayHello: (call, callback) => {
        ...
        callback(null, { message: `Hello, ${name}!` }); // Kirim response tunggal
    },
    ...
    addNumbers: (call, callback) => {
        ...
        call.on('data', (request) => {
            sum += request.number;
            console.log(`[Server] Received number: ${request.number}, current sum: ${sum}`);
        });

        // Client selesai mengirim stream
        call.on('end', () => {
            console.log(`[Server] Client finished sending numbers. Final sum: ${sum}`);
    ...
    chat: (call) => {
        ...
        // Terima data stream dari client
        call.on('data', (request) => {
            console.log(`[Server] Received chat message from ${request.sender}: ${request.message}`);
